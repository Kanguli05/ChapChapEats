        ...
        <div className="flex space-y-2 flex-col">
            {cuisineList.slice(0, isExpanded ? cuisineList.length : 7).map((cuisine) => {
                ...
                <div className="flex">
                    ...
                    <input id={`cuisine_${cuisine}`} type="checkbox" title="cuisine" placeholder=""
                    value={cuisine} checked={isSelected} onChange={handleCuisinesChange} className="hidden" />

                    <Label htmlFor={`cuisine_${cuisine}`} className={`flex flex-1 items-center cursor-pointer 
                        text-sm rounded-full px-4 py-2 font-semibold ${isSelected ? 'border border-green-600 text-green-600 ' : 'border border-slate-300'}`}>
