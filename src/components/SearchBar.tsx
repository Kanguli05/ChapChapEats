...
const formSchema = z.object({
    searchQuery: z.string({
        required_error: "Restaurant name or cuisine is required"
    })
})
...
const SearchBar = ({ onSubmit, placeholder, onReset, searchQuery}: Props) => {
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className={`flex items-center gap-3 justify-between flex-row border-2 rounded-full px-4 p-3  
                ...
                <Button type="submit" variant="outline" className="rounded-full bg-orange-500">
                    Search
                </Button>
            </form>
        </Form>
